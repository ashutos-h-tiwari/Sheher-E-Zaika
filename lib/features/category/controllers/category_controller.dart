...
import 'package:sheherezaika/features/category/domain/services/category_service_interface.dart';
...
class CategoryController extends GetxController implements GetxService {
  ...
  void getCategoryProductListSilently(String? categoryID, String type) async {
    _isRestaurant = false;
    _type = type;
    update();
    ProductModel? productModel = await categoryServiceInterface.getCategoryProductList(categoryID, 1, type);
    if (productModel != null) {
      _categoryProductList = [];
      _categoryProductList!.addAll(productModel.products!);
      _pageSize = productModel.totalSize;
  ...
  void getCategoryRestaurantListSilently(String? categoryID, String type) async {
    _isRestaurant = true;
    _type = type;
    update();
    RestaurantModel? restaurantModel = await categoryServiceInterface.getCategoryRestaurantList(categoryID, 1, type);
    if (restaurantModel != null) {
      _categoryRestaurantList = [];
      _categoryRestaurantList!.addAll(restaurantModel.restaurants!);
      _restaurantPageSize = restaurantModel.totalSize;
